        ...
        try:
            with open(output_file, 'w', encoding='utf-8', newline='') as f:
                if fmt == 'json':
                    ...
                    f.write("| Folder | Status | Return Code | Command |\n")
                    f.write("| :--- | :--- | :--- | :--- |\n")
                    for row in report_data:
                        cmd_escaped = row['command'].replace('|', '\\|')
                        f.write(f"| `{row['folder']}` | `{row['status']}` | `{row['return_code']}` | `{cmd_escaped}` |\n")

                    has_details = any(row['stdout'].strip() or row['stderr'].strip() for row in report_data)
                    if has_details:
                        f.write("\n## Details\n\n")
                        for row in report_data:
                            if row['stdout'].strip() or row['stderr'].strip():
