...
Features:
    - Generates typos based on common typing patterns (skipping letters, swapping neighbors, inserting extra letters, etc.).
...
def generate_typos_by_insertion(
    word: str,
    adjacent_keys: Mapping[str, Set[str]],
    custom_subs: Mapping[str, Set[str]] | None = None,
    use_adjacent: bool = True,
    ...
    """
    Generate typos by inserting extra characters next to the intended keys.

    Args:
        word (str): The input word.
        adjacent_keys (dict): Mapping of each character to its adjacent keys.
        custom_subs (dict, optional): Custom substitution rules.
        use_adjacent (bool): Whether to include adjacent key insertions.
        use_custom (bool): Whether to include custom insertions.

    Returns:
        set: A set of typo variations generated by insertion.
    """
    typos = set()
    ...
    for i, char in enumerate(word):
        ...
        # Adjacent key insertions (hitting two keys instead of one)
        if use_adjacent and char in adjacent_keys:
            insertion_chars.update(adjacent_keys[char])

        # Custom insertions
        if use_custom and custom_subs and char in custom_subs:
            insertion_chars.update(custom_subs[char])

        for ins_char in insertion_chars:
            # Insert before the intended character
            typos.add(word[:i] + ins_char + word[i:])
            # Insert after the intended character
            typos.add(word[:i+1] + ins_char + word[i+1:])
...
def generate_all_typos(
    ...
    # Insertion
    if typo_types.get('insertion', False):
        typos.update(
            generate_typos_by_insertion(
                word,
                adjacent_keys,
                custom_subs,
                use_adjacent,
...
""",
    ...
    gen_group.add_argument(
        '-i', '--include-insertions',
        action='store_true',
        help="Enable insertion typos (hitting two keys instead of one).",
    )
    gen_group.add_argument(
        '-m', '--min-length',
        type=int,
    ...
    if args.include_insertions:
        if 'typo_types' not in config:
            config['typo_types'] = {}
        config['typo_types']['insertion'] = True
