...
# (Optional) The number of concurrent jobs/threads to run in parallel
jobs: 4
```
...
- `-j`, `--jobs`: The number of concurrent jobs/threads to run in parallel. Defaults to 1 (sequential). Overrides config file if provided.
