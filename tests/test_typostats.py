...
# Appended Consolidated Tests (Originally from coverage gap, directory, interactive, and main test files)

def test_should_enable_color_monkeypatched(monkeypatch):
...
def test_detect_format_from_extension_consolidated():
...
def test_typostats_recursive_directory_scanning_consolidated(tmp_path, monkeypatch):
...
def test_typostats_interactive_terminal_scans_cwd_consolidated(tmp_path, monkeypatch):
...
def test_typostats_non_interactive_terminal_reads_stdin_consolidated(tmp_path, monkeypatch):
    monkeypatch.setattr("typostats._STDIN_CACHE", None)
...
def test_typostats_main_run_path_consolidated(tmp_path):
