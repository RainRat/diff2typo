...
def test_should_enable_color_respects_no_color_env(monkeypatch):
    monkeypatch.setenv("NO_COLOR", "1")
    assert typostats._should_enable_color(sys.stdout) is False
    monkeypatch.delenv("NO_COLOR")


def test_should_enable_color_respects_force_color_env(monkeypatch):
    monkeypatch.setenv("FORCE_COLOR", "1")
    assert typostats._should_enable_color(sys.stdout) is True
    monkeypatch.delenv("FORCE_COLOR")


def test_should_enable_color_with_tty_stream():
    from unittest.mock import MagicMock
...
def test_should_enable_color_with_non_tty_stream():
    from unittest.mock import MagicMock
    mock_stream = MagicMock()
...
def test_detect_format_from_extension_known():
...
def test_read_file_lines_robust_stdin_binary_latin1_fallback(monkeypatch):
    from unittest.mock import MagicMock
    monkeypatch.setattr(typostats, "_STDIN_CACHE", None)
    mock_stdin = MagicMock()
...
def test_read_file_lines_robust_empty_for_directory(tmp_path):
    dir_path = tmp_path / "test_dir"
    dir_path.mkdir()
...
def test_read_file_lines_robust_file_with_detected_latin1(tmp_path):
...
def test_read_file_lines_robust_file_with_failed_encoding_detection(tmp_path):
    file_path = tmp_path / "latin1.txt"
    with open(file_path, "wb") as f:
        f.write(b"\xe9\n")
    with patch("typostats.detect_encoding", return_value=None):
...
def test_read_file_lines_robust_file_unicode_decode_fallback(tmp_path):
...
def test_typostats_main_runs_with_basic_inputs(tmp_path):
...
def test_typostats_recursive_directory_scanning_and_ignoring_unwanted_paths(tmp_path, monkeypatch):
...
def test_typostats_main_invocation(tmp_path):
    import runpy
