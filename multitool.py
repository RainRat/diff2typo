...
def fuzzymatch_mode(
    ...
    """
    Finds pairs of similar words between two lists.
    """
    start_time = time.perf_counter()
    raw_item_count = 0
    list1_unique = []
...
def scrub_mode(
    ...
    """
    Runs replacements of typos in text files based on a mapping file or extra pairs.
...
def set_operation_mode(
    ...
    """Run set operations (intersection, union, difference) between input files (merged) and a second file."""
    start_time = time.perf_counter()
...
MODE_DETAILS = {
    "arrow": {
        "summary": "Finds text in lines with arrows (->).",
        "description": "Finds text in lines that use arrows (for example, 'typo -> correction'). It keeps the left side by default. Use --right to keep the right side instead.",
        "example": "python multitool.py arrow typos.log --right --output corrections.txt",
        "flags": "[--right]",
    },
    "table": {
        "summary": "Finds text in table-style entries (key = \"value\").",
        "description": "Finds keys or values from entries like 'key = \"value\"'. It keeps the key by default. Use --right to keep the quoted value instead.",
    ...
    "unique": {
        ...
        "description": "Removes duplicate items from your list. Unlike 'combine', it keeps the order in which items first appeared in your files.",
        "example": "python multitool.py unique raw_typos.txt --output clean_typos.txt",
        "flags": "",
    },
    "backtick": {
        "summary": "Finds text found inside backticks.",
        "description": "Finds text inside backticks (for example, `code`). It prioritizes items near words like 'error' or 'warning' to find the most useful data.",
        "example": "python multitool.py backtick build.log --output suspects.txt",
        "flags": "",
    },
    "csv": {
        "summary": "Finds specific columns in a CSV file.",
        "description": "Finds data in CSV files. By default, it finds every column except the first one. Use --first-column to find only the first column, or --column to pick specific numbers.",
        "example": "python multitool.py csv typos.csv --column 2 -o corrections.txt",
        "flags": "[--first-column] [--column NUMBER]",
    },
    "markdown": {
        "summary": "Finds items in Markdown bulleted lists.",
        "description": "Finds text in lines starting with -, *, or +. It can also split items by ':' or '->' to find one side of a pair (use --right for the second part).",
        "example": "python multitool.py markdown notes.md --output items.txt",
        "flags": "[--right]",
    },
    "md-table": {
        "summary": "Finds text in Markdown tables.",
    ...
    "json": {
        "summary": "Finds values in a JSON file using an optional key.",
        "description": "Finds values for a specific key in a JSON file. Use dots for nested keys (for example, 'user.name'). If no key is provided, it finds items from the top level. It automatically handles lists of objects.",
        "example": "python multitool.py json list.json -o items.txt",
        "flags": "[-k KEY]",
    },
    "yaml": {
        "summary": "Finds values in a YAML file using an optional key.",
        "description": "Finds values for a specific key in a YAML file. Use dots for nested keys (for example, 'config.items'). If no key is provided, it finds items from the top level. It automatically handles lists.",
    ...
    "words": {
        "summary": "Finds individual words in a file.",
        "description": "Splits a file into individual words using whitespace or a custom delimiter. It's the standard way to find every word used in a document. Use --smart to split by capital letters and symbols.",
        "example": "python multitool.py words report.txt --smart --output wordlist.txt",
        "flags": "[-d DELIMITER] [--smart]",
    },
    "ngrams": {
        "summary": "Finds sequences of N words.",
        "description": "Finds sequences of N words in a file. This is useful for finding common phrases or context around typos. It supports sequences across line boundaries.",
        "example": "python multitool.py ngrams report.txt -n 2 --smart --output phrases.txt",
        "flags": "[-n N] [-d DELIMITER] [--smart]",
    },
    "count": {
        "summary": "Shows how many times each word or pair appears.",
        "description": "Shows frequency and sorts the list from most frequent to least frequent. Use -f arrow for a rich visual report with bar charts. Use --pairs to count word pairs (for example, typo -> correction) instead of single words.",
    ...
    "map": {
        ...
        "description": "Replaces items in your list with new values from a mapping file or extra pairs provided via --add. Supports CSV, Arrow, Table, JSON, and YAML mapping formats. Use --smart-case to keep capitalization and --pairs to see both original and changed words.",
    ...
    "pairs": {
        "summary": "Runs and converts paired data.",
    ...
    "stats": {
        "summary": "Shows detailed statistics for a typo list.",
    ...
    "discovery": {
        ...
        "description": "Automatically finds potential typos in a text by finding rare words that are very similar to frequent words. It assumes that frequent words are likely correct and rare versions are likely typos. This is a powerful way to find errors without needing a dictionary.",
    ...
    "search": {
        "summary": "Finds words or patterns in text files.",
    ...
    "scrub": {
        ...
        "description": "Runs in-place replacements of typos in your text files using a mapping file or extra pairs provided via --add. It tries to keep the surrounding context (punctuation, whitespace) while fixing errors. It automatically handles compound words like 'CamelCase' and 'snake_case' variables. Supports CSV, Arrow, Table, JSON, and YAML mapping formats.",
    ...
    "rename": {
        ...
        "description": "Renames files or directories based on a typo mapping or extra pairs provided via --add. It keeps the directory structure and can automatically handle CamelCase or snake_case names using --smart-case. It handles nested renames by processing files before their parent directories.",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    io_group.add_argument(
        ...
        metavar='FORMAT',
        default='line',
        help="The format for the output (default: line). Choices: line, json, csv, markdown, md-table, arrow, table, yaml.",
    )
    io_group.add_argument(
        '-q', '--quiet',
        action='store_true',
    ...
    proc_group.add_argument(
        '-P', '--process-output',
        action='store_true',
        help="Sort the results and remove duplicates.",
    )
    proc_group.add_argument(
        '--process',
        action='store_true',
    ...
    json_options.add_argument(
        ...
        type=str,
        default='',
        help="The key path to find (for example 'items.name'). If not provided, finds from the top level.",
    )
    ...
    yaml_options.add_argument(
        ...
        type=str,
        default='',
        help="The key path to find (for example 'config.items'). If not provided, finds from the top level.",
    )
    ...
    stats_options.add_argument(
        '-p', '--pairs',
        action='store_true',
        help="Run pair-level analysis (typos vs corrections) in addition to item-level stats.",
    )
    ...
    set_options.add_argument(
        ...
        required=True,
        help='Set operation to run between the two files.',
    )
    ...
    rename_options.add_argument(
        '--in-place',
        action='store_true',
        help="Run the actual renaming of files and directories.",
    )
    rename_options.add_argument(
        '--dry-run',
        action='store_true',
