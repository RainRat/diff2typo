...
- **`frontmatter`**
  - Extracts metadata from the start of Markdown files (between `---` or `+++` markers). It saves the raw content by default.
  - **Options:** Use the `-k` (or `--key`) flag to extract specific values from the YAML or TOML data.
