...
| `--format`, `-f` | `arrow` | Choose the output format: `arrow` (typo -> fix), `csv` (typo,fix), `table` (typo = "fix"), `list` (typo only), `json`, `yaml`, `markdown`, `md`, `html`, or `htm`. Automatically detected from file extension (`.json`, `.yaml`, `.yml`, `.md`, `.markdown`, `.html`, `.htm`). |
...
**Export typos as JSON, YAML, Markdown, or HTML:**
...
python diff2typo.py feature.diff --format html --output typos.html
```
