...
def format_typos(
    ...
    Returns:
        list or dict: Formatted typo data.
    """
    if output_format in ('json', 'yaml'):
        return dict(typo_to_correct_word)
...
""",
    ...
    io_group.add_argument(
        ...
        help="The format of the output. Options: arrow, csv, table, list, json, or yaml.",
    )
    io_group.add_argument(
        '-s', '--substitutions',
        type=str,
    ...
    try:
        ...
        if output_target == '-':
            # Write to stdout
            if settings.output_header and settings.output_format not in ('json', 'yaml'):
                print(settings.output_header)

            if settings.output_format == 'json':
                json.dump(formatted_typos, sys.stdout, indent=2)
                print()
            elif settings.output_format == 'yaml':
                yaml.dump(formatted_typos, sys.stdout, default_flow_style=False)
                print()
            else:
            ...
            with open(output_target, 'w', encoding='utf-8') as file:
                ...
                if settings.output_format == 'json':
                    json.dump(formatted_typos, file, indent=2)
                    file.write("\n")
                elif settings.output_format == 'yaml':
                    yaml.dump(formatted_typos, file, default_flow_style=False)
                    file.write("\n")
                else:
                    for typo in formatted_typos:
                        file.write(f"{typo}\n")
