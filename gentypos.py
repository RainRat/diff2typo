...
def generate_typos_by_insertion(
    ...
    """
    Generate typos by inserting characters from adjacent keys or custom substitutions.
    ...
    # Iterate through every possible insertion point (including start and end)
    for i in range(len(word) + 1):
        # We look at the character before the insertion point (if any)
        # and the character after the insertion point (if any).
        chars_to_check = []
        if i > 0:
            chars_to_check.append(word[i-1])
        if i < len(word):
            chars_to_check.append(word[i])
        ...
        for char in chars_to_check:
            ...
            # Custom substitutions (single character)
            if use_custom and custom_subs and char in custom_subs:
                for sub in custom_subs[char]:
                    if len(sub) == 1:
                        insertion_chars.add(sub)

        for ins_char in insertion_chars:
            typo = word[:i] + ins_char + word[i:]
            typos.add(typo)
...
""",
    ...
    gen_group.add_argument(
        ...
        action='store_true',
        help="Include insertion typos (e.g., 'wpoard' instead of 'word').",
    )
    gen_group.add_argument(
        '-v', '--verbose',
        action='store_true',
