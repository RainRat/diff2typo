...
Features:
    - Generates typos based on common typing patterns (skipping letters, swapping neighbors, inserting adjacent keys, etc.).
...
def generate_typos_by_insertion(
    ...
    """
    Generate typos by inserting adjacent keys or custom substitutions.
    ...
    # Insert at every possible position in the word
    for i in range(len(word) + 1):
        # Characters adjacent to the current insertion point
        relevant_chars = set()
        if i < len(word):
            relevant_chars.add(word[i])
        if i > 0:
            relevant_chars.add(word[i - 1])

        insertion_candidates = set()
        for char in relevant_chars:
            if use_adjacent and char in adjacent_keys:
                insertion_candidates.update(adjacent_keys[char])
            if use_custom and custom_subs and char in custom_subs:
                insertion_candidates.update(custom_subs[char])

        for ins_char in insertion_candidates:
