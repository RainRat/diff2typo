...
- **Backward Compatibility:** Do not write in user-facing docs that a feature is "new". References to "new features" become obsolete quickly.
- **Dependencies:** Minimize external dependencies. Prefer standard library (`argparse`, `logging`, `csv`, `json`) where possible.
- Update or add unit tests alongside code changes when behavior changes.
- **Typo Generation Strategy:** Reject any proposal for "fake" typo generation using insertions. The 1 to 2 character replacement is more focused than inserting arbitrary characters.
...
## Coding Standards
- **Type Hinting:** Use `typing` (List, Dict, Optional, and so on) for function signatures.
