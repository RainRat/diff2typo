...
**Multitool** is a multipurpose tool for cleaning, getting, and checking text files.
...
- **`arrow`**
  - **What it does:** Finds text in lines that use arrows (for example, `typo -> correction`). Useful for getting a clean list of typos from a log. It keeps the left side by default. Use the `--right` flag to keep the right side instead.
  - **Example:** `python multitool.py arrow typos.log --right`

- **`table`**
  - **What it does:** Finds keys or values from entries like `key = "value"`. It keeps the key by default. Use the `--right` flag to keep the quoted value instead.
  - **Example:** `python multitool.py table typos.toml --right`

- **`backtick`**
  - **What it does:** Finds text found inside backticks (for example, \`code\`). It prioritizes items near words like 'error' or 'warning' to find the most useful data.
  - **Example:** `python multitool.py backtick build.log`

- **`csv`**
  - **What it does:** Finds specific columns in a CSV file. By default, it finds **every column except the first one**. Use `--first-column` to find *only* the first column, or `--column` (or `-c`) followed by one or more numbers to pick specific columns. Use `--delimiter` (or `-d`) to pick a different column separator (for example, `;`).
  - **Example:** `python multitool.py csv data.csv --column 2`

- **`markdown`**
  - **What it does:** Finds items in Markdown bulleted lists (lines starting with `- `, `* `, or `+ `). It can also split items by `:` or `->` to find one side of a pair (use the `--right` flag for the second part).
  - **Example:** `python multitool.py markdown notes.md --right`

- **`md-table`**
  - **What it does:** Finds text in Markdown tables. It keeps the first column by default. Use the `--right` flag to keep the second column instead, or `--column` (or `-c`) followed by one or more numbers to pick specific columns. It automatically skips header and divider rows.
  - **Example:** `python multitool.py md-table readme.md --column 2`

- **`json`**
  - **What it does:** Finds values in a JSON file using an optional key. You can use dots for nested keys (for example, `user.name`). If no key is provided, it finds items from the top level. It automatically handles lists and objects.
  - **Example:** `python multitool.py json report.json --key replacements.typo`

- **`yaml`**
  - **What it does:** Finds values in a YAML file using an optional key. Like JSON mode, it supports dot notation (for example, `config.items`). If no key is provided, it finds items from the top level.
...
- **`words`**
  - **What it does:** Finds individual words in a file. It splits lines by whitespace by default, but you can pick a custom character using `--delimiter` (or `-d`). Use the `--smart` (or `-S`) flag to also split by symbols and capital letters (for example, splitting "CamelCase" into "Camel" and "Case").
  - **Example:** `python multitool.py words report.txt --smart`

- **`ngrams`**
  - **What it does:** Finds sequences of N words in a file. This is useful for finding common phrases or context around typos. It supports sequences across line boundaries.
...
- **`regex`**
  - **What it does:** Finds text that matches a pattern (regular expression). Unlike other modes, it **keeps the original text** (it does not convert to lowercase or remove punctuation) by default.
...
- **`combine`**
  - **What it does:** Merges multiple files (or standard input) into one clean list, removes duplicates, and sorts the result alphabetically.
...
- **`unique`**
  - **What it does:** Removes duplicates while **keeping the original order**. This is useful when the sequence of items is important.
...
- **`rename`**
  - **What it does:** Renames files and directories using a mapping file or extra pairs. It is useful for fixing typos in filenames across your entire project. It handles nested renames by processing files before their parent folders.
  - **Options:**
...
- **`diff`**
  - **What it does:** Finds added, removed, or changed items between files. It can compare simple lists of words or (with the `--pairs` flag) find changes in typo-correction mappings.
...
- **`zip`**
  - **What it does:** Pairs lines from two files together. It applies `--min-length` and `--max-length` filters to **both items in each pair**. If the files have a different number of lines, the output will stop at the end of the shortest file.
...
- **`scrub`**
  - **What it does:** Replaces typos in text files based on a mapping or extra pairs. It tries to keep the surrounding context (punctuation, whitespace) while fixing errors. It automatically handles compound words like `CamelCase` and `snake_case` variables.
...
- **`standardize`**
  - **What it does:** Fixes inconsistent casing and spelling using the most frequent form. It analyzes your files to find words used with different capitalization (for example, 'database' vs 'Database') or similar spelling (for example, 'teh' vs 'the'). It then automatically replaces all less frequent versions with the most popular one across the entire project. This ensures naming consistency without needing a manual mapping file.
  - **Options:**
...
- **`highlight`**
  - **What it does:** Highlights words from a file or extra pairs. This is useful as a preview before using the `scrub` mode to make permanent changes.
...
- **`pairs`**
  - **What it does:** Runs and converts paired data (like `typo -> correction`) from any supported format and writes it to the specified output format. This is the primary way to convert between paired formats (for example, from JSON to CSV) while applying cleaning and length filters.
  - **Supported Formats:** `arrow`, `table`, `csv`, `markdown`, `md-table`, `json`, and `yaml`.
...
- **`check`**
  - **What it does:** Finds words that are both typos and corrections. This is useful for spotting errors in your typo database (loops).
...
- **`conflict`**
  - **What it does:** Finds typos that have multiple different corrections. Use this to find inconsistencies in your typo lists.
  - **Supported Formats:** `arrow`, `table`, `csv`, `markdown`, `md-table`, `json`, and `yaml`.
  - **Example:** `python multitool.py conflict mappings.csv`

- **`count`**
  - **What it does:** Shows how many times each word or pair appears and sorts them by frequency (most frequent first).
  - **Options:**
...
- **`fuzzymatch`**
  - **What it does:** Finds similar words between two lists. Use this to find likely corrections for typos.
...
- **`near_duplicates`**
  - **What it does:** Finds similar words in a single list (only a few characters are different). This is useful for finding potential typos or unintended duplicates.
  - **Options:** Use `--min-dist` and `--max-dist` to control the number of changes allowed, and `--show-dist` to see the number of changes in the output.
  - **Supported Formats:** `arrow`, `table`, `csv`, `markdown`, `md-table`, `json`, and `yaml`.
  - **Example:** `python multitool.py near_duplicates words.txt --max-dist 1 --show-dist`

- **`similarity`**
  - **What it does:** Filters paired data by the number of changes needed to turn one word into another. Use this to remove extra data or find specific types of typos.
...
- **`stats`**
  - **What it does:** Shows detailed statistics for a typo list. It reports counts, unique items, and statistics. If the `--pairs` flag is used, it additionally analyzes the file as paired data (typos/corrections) and reports conflicts (one typo to multiple corrections), overlaps (words that are both typos and corrections), and character change statistics.
  - **Supported Formats:** `json`, `yaml`, `markdown`, `md-table`, and `line` (human-readable).
  - **Example:** `python multitool.py stats typos.csv --pairs`

- **`discovery`**
  - **What it does:** Finds typos by comparing rare and frequent words. It assumes that frequent words are likely correct and rare variations are likely typos. This is a powerful way to find errors in a dataset without needing a dictionary.
  - **Options:**
...
- **`casing`**
  - **What it does:** Finds words with inconsistent capitalization (for example, 'hello', 'Hello', 'HELLO'). This is useful for identifying inconsistent naming or typos that differ only by case.
  - **Options:**
    - `-d`, `--delimiter`: The character to split words by (default: whitespace).
    - `-S`, `--smart`: Split by symbols and capital letters (for example, splitting "CamelCase" into "Camel" and "Case").
  - **Supported Formats:** `arrow`, `table`, `csv`, `markdown`, `md-table`, `json`, and `yaml`.
...
- **`search`**
  - **What it does:** Finds words or patterns in text files. It can find similar words (typos) or subword matches.
  - **Options:**
...
- **`scan`**
  - **What it does:** Scans for words or typos from a file or extra pairs and reports all matches with filename, line number, and highlighting. This is the recommended way to check your project for a set of known typos before performing replacements.
