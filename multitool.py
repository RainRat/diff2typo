...
def codeblocks_mode(
    ...
    for input_file in input_files:
        for cb_lang, cb_content in _extract_markdown_codeblocks(input_file, quiet=quiet):
            ...
            content_to_save = filter_to_letters(cb_content) if clean_items else cb_content
            if not (min_length <= len(content_to_save) <= max_length):
                continue

            if pairs:
                results.append((cb_lang, content_to_save))
            else:
                results.append(content_to_save)
